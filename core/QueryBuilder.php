...
class QueryBuilder{
    ...
    public function registerAdmin($email, $passsword, $name){
        $query = $this->pdo->prepare('select * from admin');
        $query->execute();
        $result = $query->fetchAll(\PDO::FETCH_OBJ);
        ...
        $query = $this->pdo->prepare('insert into `admin` values ("' .$id.'", "'.$email.'", "'.$passsword.'", "'.$name.'")');
        $query->execute();
    }
    ...
    public function setStudentBehaviorLevel($studentID, $behaivorLevel, $recorder){
